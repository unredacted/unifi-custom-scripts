...
# -----------------------------------------------------------------
# apply_nud_settings: enforce 4-hour NUD window on an IXP bridge
#
# IXP peering fabrics (AMS-IX, SIX, HOUIX, KCIX, …) require a
# 4-hour ARP/ND cache timeout.  UBIOS recreates bridge interfaces
# during reprovisioning, which resets per-interface neigh parameters
# back to the kernel default (base_reachable_time_ms = 30 000 ms).
#
# At the kernel default, a 60-second BGP keepalive arrives after the
# NUD entry has already gone STALE, triggering the
#   STALE → DELAY → PROBE → broadcast-ARP
# cycle — exactly the 90-second storm that IX NOCs report.
#
# We cannot rely on inject-rules.conf alone because it runs at boot
# before UBIOS has brought up the bridge interfaces, so the sysctl
# calls find no /proc/sys/net/{ipv4,ipv6}/neigh/<iface>/ directory
# and silently succeed without writing anything.
#
# By running this check every monitor cycle (default 30 s) we catch
# the window between interface-up and the next inject-rules re-run.
#
# Targets (per AMS-IX / SIX policy):
#   base_reachable_time_ms = 14 400 000  (4 h, in milliseconds)
#   gc_stale_time          = 14 400      (4 h, in seconds)
# -----------------------------------------------------------------
apply_nud_settings() {
    local iface="$1"

    # Interface may not be up yet — skip silently and retry next cycle.
    if [[ ! -d "/proc/sys/net/ipv4/neigh/${iface}" ]]; then
        echo "[neighbor-poll-wrapper] NUD: ${iface} not yet present, will retry."
        return 0
    fi

    local current_ms
    current_ms=$(cat "/proc/sys/net/ipv4/neigh/${iface}/base_reachable_time_ms" 2>/dev/null || echo 0)

    if [[ "$current_ms" != "14400000" ]]; then
        echo "[neighbor-poll-wrapper] NUD: ${iface} base_reachable_time_ms=${current_ms} — applying 4-hour settings."
        sysctl -w "net.ipv4.neigh.${iface}.base_reachable_time_ms=14400000" 2>&1 || true
        sysctl -w "net.ipv6.neigh.${iface}.base_reachable_time_ms=14400000" 2>&1 || true
        sysctl -w "net.ipv4.neigh.${iface}.gc_stale_time=14400"             2>&1 || true
        sysctl -w "net.ipv6.neigh.${iface}.gc_stale_time=14400"             2>&1 || true
    else
        echo "[neighbor-poll-wrapper] NUD: ${iface} already has 4-hour settings (${current_ms} ms)."
    fi
}

echo "[neighbor-poll-wrapper] Applying NUD settings to IXP bridges..."
for iface in "${INTERFACES[@]}"; do
    apply_nud_settings "$iface" || true
done
