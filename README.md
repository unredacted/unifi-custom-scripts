...
Applies iptables, ebtables, ip rule, ip route, and route-sync directives idempotently from a config file. A companion daemon monitors for kernel events that indicate rules were flushed (e.g., UBIOS provisioning cycles) and re-applies them automatically.

**The problem:** UniFi OS periodically re-provisions the network stack, which flushes custom iptables rules, policy routes, and routing table entries. These scripts solve that by applying rules idempotently and monitoring for flushes to restore them.
